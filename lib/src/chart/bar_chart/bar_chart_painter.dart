...
class BarChartPainter extends AxisChartPainter<BarChartData> with TouchHandler<BarTouchResponse> {
  ...
  void _drawDash(CanvasWrapper canvasWrapper, List<_GroupBarsPosition> groupBarsPosition) {
    final viewSize = canvasWrapper.size;
    ...
    for (var i = 0; i < data.barGroups.length; i++) {
      ...
      for (var j = 0; j < barGroup.barRods.length; j++) {
        ...
        for (var i = 1; dashLength * i <= dashHeight; i++) {
          ...
          dashPath.moveTo(dashX, dashLength * ++i);
        }
        dashXFinal = x + x0 / 2;
        canvasWrapper.drawPath(dashPath, paint);
        // canvasWrapper.drawRRect(barRRect, paint);
    ...
    if (leftTitles.showTitles) {
      ...
      while (verticalSeek <= data.maxY) {
        if (leftTitles.checkToShowTitle(data.minY, data.maxY, leftTitles, leftInterval, verticalSeek)) {
          ...
          final span = TextSpan(style: TextStyle(fontSize: 15), text: text);
  ...
  BarTouchedSpot? _getNearestTouchedSpot(
    ...
    for (var i = 0; i < groupBarsPosition.length; i++) {
      ...
      for (var j = 0; j < groupBarPos.barsX.length; j++) {
        ...
        final backDrawBarY = getPixelY(targetData.barGroups[i].barRods[j].backDrawRodData.y, chartViewSize);
        final touchExtraThreshold = targetData.barTouchData.touchExtraThreshold;

        final isXInTouchBounds = (touchedPoint.dx <= barX + halfBarWidth + touchExtraThreshold.right) &&
